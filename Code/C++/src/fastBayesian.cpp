...
#include "densityKernel.h"
 ...
 int main(int argc, char **argv)
 {
   ...
   double tL[3], beta[3], corr[2], pathIntU[3], u0[3] = {}, sigma_u[3]; 
   ...
   double dQ = Q*dt[0]/(ppr*noProcs);
   pathIntU[0] = Uvec0[0]*dt[0]; //dt[1]
   pathIntU[1] = Uvec0[1]*dt[0]; //dt[1]
   pathIntU[2] = windModel->Umag(zt[0])*dt[0];
   ...
   double* gradDrift = new double[ppr*nor]();
   double* C = new double[nor*noStations]();
   double* JacMat = new double[2*nor*noStations]();
   double* HessMat = new double[4*nor*noStations]();
   ...
   do {
	...
	while ( ceil(tp*1000)/1000 < tm || tp_z < tm )
	{
		...
		zdot[1] = corr[1]*(zdot[0] + A0*dt[1] + beta[2]*sqrt(dt[1])*xsi[kk]);
	    }
	    else
	    {
		...
		pathIntU[2] += windModel->Umag(zz)*dt[0];
            }
...
//	  xdot_eject[0] = -Uvec0[0]; xdot_eject[1] = -Uvec0[1];
	...
	else 
	{
	  ...
	  windModel->updateParams(tm/mfreq-1); eddyDiffModel->updateParams(tm/mfreq-1);
...

        {
         ...
         double h[2], Cda, sine, cosine, wDirNoise, mesoNoiseSD, mesoNoiseVar; 
         if ( newPuff )
         {
           h[1] = bandWidths[nor]; Cda = 4.0/3.0; sine = sin(windModel->wdir(0)); cosine = cos(windModel->wdir(0)); wDirNoise=windModel->windDirFluc(0); mesoNoiseSD = windModel->mesoFlucSD(0); 
           mesoNoiseVar = mesoNoiseSD*mesoNoiseSD; 
         }
         while ( newPuff && i < nor*noStations )
         {
            if ( nocp[r] == 0 && i + noStations < nor*noStations )
            {
                ...
                h[1] = bandWidths[nor+r]; Cda = 4.0/3.0; sine = sin(windModel->wdir(r)); cosine = cos(windModel->wdir(r)); wDirNoise=windModel->windDirFluc(r); mesoNoiseSD = windModel->mesoFlucSD(r); mesoNoiseVar = mesoNoiseSD*mesoNoiseSD; 
                continue;
            }
            else if ( nocp[r] == 0 )
            {
	   ...
	   if ( mesoNoiseVar == 0 )
	   {
	      dJCxy = -sine*gradDrift[j/3]*((stations[ii]+positions[j])*(cosine*cosine/pathVar[(j*2)/3] + sine*sine/pathVar[(j*2)/3+1])+(stations[ii+1]+positions[j+1])*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]));
	      dJCxy += cosine*gradDrift[j/3]*((stations[ii]+positions[j])*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1])+(stations[ii+1]+positions[j+1])*(sine*sine/pathVar[(j*2)/3] + cosine*cosine/pathVar[(j*2)/3+1]));
	      dJCxy -= (stations[ii]+positions[j])*(stations[ii]+positions[j])*sine*cosine*(-1/pathVar[(j*2)/3] + 1/pathVar[(j*2)/3+1]);
	      dJCxy -= (stations[ii+1]+positions[j+1])*(stations[ii+1]+positions[j+1])*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]);
	      dJCxy -= (stations[ii]+positions[j])*(stations[ii+1]+positions[j+1])*(2*cosine*cosine-1)*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]);
	      dJCxy *= Pi/180;

	      dHCxy = -sine*sine*gradDrift[j/3]*gradDrift[j/3]*(cosine*cosine/pathVar[(j*2)/3] + sine*sine/pathVar[(j*2)/3+1]);
	      dHCxy -= cosine*cosine*gradDrift[j/3]*gradDrift[j/3]*(sine*sine/pathVar[(j*2)/3] + cosine*cosine/pathVar[(j*2)/3+1]);
	      dHCxy += sine*cosine*gradDrift[j/3]*gradDrift[j/3]*2*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]);
	      dHCxy -= cosine*gradDrift[j/3]*((stations[ii]+positions[j])*(cosine*cosine/pathVar[(j*2)/3] + sine*sine/pathVar[(j*2)/3+1])+(stations[ii+1]+positions[j+1])*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]));
	      dHCxy -= sine*gradDrift[j/3]*((stations[ii]+positions[j])*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1])+(stations[ii+1]+positions[j+1])*(sine*sine/pathVar[(j*2)/3] + cosine*cosine/pathVar[(j*2)/3+1]));
	      dHCxy -= sine*gradDrift[j/3]*((stations[ii]+positions[j])*2*sine*cosine*(-1/pathVar[(j*2)/3] + 1/pathVar[(j*2)/3+1])+(stations[ii+1]+positions[j+1])*(2*cosine*cosine-1)*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]));
	      dHCxy += cosine*gradDrift[j/3]*((stations[ii]+positions[j])*(2*cosine*cosine-1)*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1])+(stations[ii+1]+positions[j+1])*2*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]));
	      dHCxy -= (stations[ii]+positions[j])*(stations[ii]+positions[j])*(2*cosine*cosine-1)*(-1/pathVar[(j*2)/3] + 1/pathVar[(j*2)/3+1]);
	      dHCxy -= (stations[ii+1]+positions[j+1])*(stations[ii+1]+positions[j+1])*(2*cosine*cosine-1)*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]);
	      dHCxy += 2*(stations[ii]+positions[j])*(stations[ii+1]+positions[j+1])*2*sine*cosine*(1/pathVar[(j*2)/3] - 1/pathVar[(j*2)/3+1]);
	      dHCxy *= Pi*Pi/(180*180);
	    }

	   double dist2 = (stations[ii+2] - positions[j+2])*(stations[ii+2] - positions[j+2])/(h[1]*h[1]);

           if ( dist2 < 1 && ( mesoNoiseVar == 0 ))
           {
              double z = 1-dist2; 
              C[jj] += dQ*dCxy*z/(h[1]*Cda);
	      JacMat[2*jj] += dQ*dCxy*z/(h[1]*Cda)*dJCxy;
              HessMat[4*jj] += dQ*dCxy*z/(h[1]*Cda)*(dJCxy*dJCxy+dHCxy);
           }
           else if ( dist2 < 1 )
           {
             double z = 1-dist2; 
             C[jj] += dQ*dCxy*z/(h[1]*Cda);
             JacMat[2*jj] += dQ*dCxy*z/(h[1]*Cda)*wDirNoise/mesoNoiseVar*Pi/180;
             JacMat[2*jj+1] += dQ*dCxy*z/(h[1]*Cda)*(wDirNoise*wDirNoise/(mesoNoiseVar*mesoNoiseSD)-1/mesoNoiseSD)*Pi/180;
             HessMat[4*jj] += dQ*dCxy*z/(h[1]*Cda)*(wDirNoise*wDirNoise/mesoNoiseVar-1)/mesoNoiseVar*Pi*Pi/(180*180);
             HessMat[4*jj+1] += dQ*dCxy*z/(h[1]*Cda)*wDirNoise*(wDirNoise*wDirNoise/mesoNoiseVar-3)/(mesoNoiseVar*mesoNoiseSD)*Pi*Pi/(180*180);
             HessMat[4*jj+2] += dQ*dCxy*z/(h[1]*Cda)*wDirNoise*(wDirNoise*wDirNoise/mesoNoiseVar-3)/(mesoNoiseVar*mesoNoiseSD)*Pi*Pi/(180*180);
	     HessMat[4*jj+3] += dQ*dCxy*z/(h[1]*Cda)*(wDirNoise*wDirNoise*(wDirNoise*wDirNoise/mesoNoiseVar-6)/(mesoNoiseVar*mesoNoiseVar)+3/mesoNoiseVar)*Pi*Pi/(180*180);
           }
	...
	if ( newPuff && noProcs > 1 )
	{
	  ...
	  fill(gradDrift,gradDrift+ppr*nor,'\0');
	  windModel->windDirFlucReset();
          eddyDiffModel->timeStep(dt, zt[0]);
	  windModel->updateWindDirFluc(); 
          dQ = Q*dt[0]/(ppr*noProcs);
          MPI_Barrier(MPI_COMM_WORLD);
        }
   ...
   {
     double* CContainer = new double[4*nor*noStations*noProcs];
     ...
     MPI_Barrier(MPI_COMM_WORLD);
     MPI_Gather(JacMat, 2*nor*noStations, MPI_DOUBLE, CContainer, 2*nor*noStations, MPI_DOUBLE, master, MPI_COMM_WORLD);
     i = 0;
     while ( i < nor*noStations*noProcs )
     {
        if ( i/(nor*noStations) == master )
        ...
        unsigned long ii = i % (nor*noStations);
        JacMat[2*ii] += CContainer[2*i];
	JacMat[2*ii+1] += CContainer[2*i+1];
        i++;
     }

     MPI_Barrier(MPI_COMM_WORLD);
     MPI_Gather(HessMat, 4*nor*noStations, MPI_DOUBLE, CContainer, 4*nor*noStations, MPI_DOUBLE, master, MPI_COMM_WORLD);
     i = 0;
     while ( i < nor*noStations*noProcs )
     {
        if ( i/(nor*noStations) == master )
        ...
        unsigned long ii = i % (nor*noStations);
        HessMat[4*ii] += CContainer[4*i];
	HessMat[4*ii+1] += CContainer[4*i+1];
	HessMat[4*ii+2] += CContainer[4*i+2];
	HessMat[4*ii+3] += CContainer[4*i+3];
        i++;
     }
     delete[] CContainer;
   }
   else if ( noProcs > 1 )
   {
     ...
     MPI_Barrier(MPI_COMM_WORLD);
     MPI_Gather(JacMat, 2*nor*noStations, MPI_DOUBLE, NULL, 2*nor*noStations, MPI_DOUBLE, master, MPI_COMM_WORLD);     
     MPI_Barrier(MPI_COMM_WORLD);
     MPI_Gather(HessMat, 4*nor*noStations, MPI_DOUBLE, NULL, 4*nor*noStations, MPI_DOUBLE, master, MPI_COMM_WORLD);
   }
  ...
  MPI_Finalize();
   ...
   if ( isMaster )
   {
     ...
     ofstream ofile_jac("output/JAC_" + windModel->meteofile());
     ofstream ofile_hess("output/HESS_" + windModel->meteofile());
     for (unsigned i = 0; i < noStations*nor; i++)
     {
        ofile << i/noStations << setw(15) << i % noStations << setw(15) << C[i] << endl; // << " " << setprecision(16)
	ofile_jac << i/noStations << setw(15) << i % noStations << setw(15) << JacMat[2*i] << setw(15) << JacMat[2*i+1] << endl;
	ofile_hess << i/noStations << setw(15) << i % noStations << setw(15) << HessMat[4*i] << setw(15) << HessMat[4*i+1] << setw(15) << HessMat[4*i+2] << setw(15) << HessMat[4*i+3] << endl;
     }
     ofile.close();
     ofile_jac.close();
     ofile_hess.close();
   ...
   delete windModel, delete eddyDiffModel, delete[] C, delete[] JacMat, delete[] HessMat, delete[] stations, delete[] meanDrift, delete[] positions, delete[] pathVar, delete[] gradDrift;
